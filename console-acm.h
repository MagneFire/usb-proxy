...
// next to usb-proxy; the function lives in-process, as part of a gadget whose
// ep0 handler hands it the requests for its interfaces and endpoints and
// starts/stops it with the configuration. The bytes go to/from a pty whose
// other end runs a shell (console-shell.cpp).
...
struct AcmFunction {
	...
	bool reserved = false;
	// Interface numbers in the gadget's configuration (set by the gadget that
	// carries the function). 0xff = not placed yet: nothing is answered.

	...
	if (recip == USB_RECIP_INTERFACE) {
		...
		if (type == USB_TYPE_STANDARD) {
			...
			case USB_REQ_GET_STATUS:
				ep0_reply(fd, "console", zero2, 2, ctrl->wLength);
				return true;
			case USB_REQ_GET_INTERFACE:
				ep0_reply(fd, "console", zero2, 1, ctrl->wLength);
				return true;
		...
		if (type == USB_TYPE_CLASS) {
			...
			case USB_CDC_REQ_GET_LINE_CODING:
				ep0_reply(fd, "console", g_acm.line_coding, 7, ctrl->wLength);
				return true;

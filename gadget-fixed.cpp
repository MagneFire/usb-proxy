...
void handle_control(int fd, struct usb_ctrlrequest *ctrl)
{
	...
	if (recip == USB_RECIP_INTERFACE) {
		...
		case USB_REQ_GET_STATUS:
			ep0_reply(fd, "fixed gadget", zero2, 2, ctrl->wLength);
			return;
		case USB_REQ_GET_INTERFACE:
			ep0_reply(fd, "fixed gadget", zero2, 1, ctrl->wLength);
			return;
		case USB_REQ_SET_INTERFACE:
			if ((ctrl->wValue & 0xff) == 0)
				ep0_ack(fd, "fixed gadget");
			else
				ep0_stall(fd, "fixed gadget");
			return;
		default:
	...
	if (recip == USB_RECIP_ENDPOINT) {
		...
		case USB_REQ_GET_STATUS: {
			...
			ep0_reply(fd, "fixed gadget", st, 2, ctrl->wLength);
			return;
		}
	...
	case USB_REQ_GET_DESCRIPTOR: {
		...
		ep0_reply(fd, "fixed gadget", buf, n, ctrl->wLength);
		return;
	}
	...
	case USB_REQ_GET_CONFIGURATION:
		ep0_reply(fd, "fixed gadget", &g_fixed.configured, 1, ctrl->wLength);
		return;
	case USB_REQ_GET_STATUS:
		ep0_reply(fd, "fixed gadget", zero2, 2, ctrl->wLength);
		return;

...
public:
	...
	bool out_feed(struct thread_info *ti, const uint8_t *data, int len)
	{
		...
		if (len == 24 && out.at_boundary()) {
			...
			if (adb_parse_hdr(data, &cmd, &arg0, &arg1, &plen) &&
			    cmd == OKAY && plen == 0) {
		...
		while (off < len) {
			...
			out.hdr_have = 0;
			uint32_t plen;
			if (!adb_parse_hdr(out.hdr, &out.cmd, &out.arg0, &out.arg1, &plen)) {
				die("unparseable header on host->device stream");
				return false;
			}
			if (verbose_level > 0 && out.cmd != WRTE) {
				char name[5];
				adb_cmd_to_string(out.cmd, name);
			...
			int nbytes)
	{
		...
		if (nbytes == 24 && in.at_boundary()) {
			uint32_t cmd, arg0, arg1, plen;
			if (adb_parse_hdr((uint8_t *)io.data, &cmd, &arg0, &arg1, &plen) &&
			    cmd == OKAY && plen == 0) {
		...
		while (off < nbytes) {
			...
			in.hdr_have = 0;
			uint32_t plen;
			if (!adb_parse_hdr(in.hdr, &in.cmd, &in.arg0, &in.arg1, &plen)) {
...
static std::atomic<bool> ack_accel_enabled(true);

void ack_accel_set_enabled(bool enabled)
{
	ack_accel_enabled = enabled;
}

// The accelerator runs only when asked for and allowed on the current pair.
static bool ack_accel_active(void)
{
	return adb_ack_accel && ack_accel_enabled;
}
		...
		if (ep.bEndpointAddress & USB_DIR_IN) {
			...
			if ((ep.bmAttributes & USB_ENDPOINT_XFERTYPE_MASK) == USB_ENDPOINT_XFER_ISOC) {
				...
				if (rv == LIBUSB_SUCCESS ||
					...
					bool enqueued_by_accel = ack_accel_active() &&
						(ep.bmAttributes & USB_ENDPOINT_XFERTYPE_MASK) == USB_ENDPOINT_XFER_BULK &&
...
void ep0_loop(int fd) {
	...
	// wedging the gadget half-configured. Only this thread's mask changes; the
	// power_monitor thread keeps its own.
	block_incidental_signals(false);

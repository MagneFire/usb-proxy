...
// console's) once the raw-gadget fd is running. Shared by the transparent
// path and the persistent gadget.
int remap_host_endpoints_if_needed(int fd);

// Whether the adb ack accelerator (--adb_ack_accel) may act on the bulk pair
// that eps_start() is about to run. The transparent path leaves it on; the
// persistent gadget turns it off for the fastboot slot, whose stream is not
// adb and would only make the accelerator fail open with a log line.
